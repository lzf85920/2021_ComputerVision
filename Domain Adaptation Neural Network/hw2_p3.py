...
if (t_name == 'MNIST-M') or (t_name == 'mnistm'):
  ...
  dann.load_state_dict(torch.load('./DANN_SVHN_to_MNIST-M_best.pth?dl=1'))
  print('load model from : ./DANN_SVHN_to_MNIST-M_best.pth?dl=1' )
  dann.to(device)
  ...
  dann = MNISTMmodel()
  dann.load_state_dict(torch.load('./MNIST-M_to_USPS_best.pth?dl=1'))
  print('load model from : ./MNIST-M_to_USPS_best.pth?dl=1' )
  dann.to(device)
  test_tfm = transforms.Compose([
  ...
  dann = USPSMmodel()
  dann.load_state_dict(torch.load('./USPS_to_SVHN_best.pth?dl=1'))
  dann.to(device)
  print('load model from : ./USPS_to_SVHN_best.pth?dl=1' )
  test_tfm = transforms.Compose([
  transforms.Resize((28,28)),
  transforms.ToTensor(),
  transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
